(() => {
  ...
  const THEME_COLORS = { light: '#f4efe5', dark: '#111614' };
  const state = {
    screen: 'inicio', search: '', company: 'TODOS', category: 'TODOS', type: 'TODOS',
    visible: PAGE_SIZE, current: null, galleryIndex: 0, theme: 'light'
  };
  ...
  function applyTheme(theme, persist = true) {
    ...
    if (btn) {
      ...
      $('.theme-toggle-icon', btn).textContent = isDark ? '☀' : '☾';
    }
  ...
  function clearAll() {
    state.search = '';
    state.company = state.category = state.type = 'TODOS';
  ...
  function openProduct(code) {
    ...
    $('#modal-code').textContent = `Cód. ${p.cod}`;
    $('#modal-company').textContent = p.empresa || '';
    $('#modal-title').textContent = p.nome || '';
    $('#modal-description').textContent = p.desc || '';
  ...
  function openSiblingProduct(step) {
    if (!state.current) return;
    const list = currentList();
    const index = list.findIndex(item => String(item.cod) === String(state.current.cod));
    if (index === -1 || !list.length) return;
    const next = list[(index + step + list.length) % list.length];
    openProduct(next.cod);
  }

  function productLink(p) { return catalogUrl() + `?produto=${encodeURIComponent(p.cod)}#catalogo`; }
  function currentImageUrl(p) { return new URL((p.imgs && (p.imgs[state.galleryIndex] || p.imgs[0])) || '', location.href).href; }

  async function fileFromImage(url, baseName) {
    try {
      const res = await fetch(url);
      const blob = await res.blob();
      const ext = blob.type.includes('png') ? 'png' : blob.type.includes('jpeg') ? 'jpg' : 'webp';
      return new File([blob], `${baseName}.${ext}`, { type: blob.type || 'image/webp' });
    } catch {
      return null;
    }
  }
  ...
  async function shareProduct() {
    const p = state.current; if (!p) return;
    const imageUrl = currentImageUrl(p);
    const link = productLink(p);
    const text = `*${p.nome}*\nCód. ${p.cod} · ${p.empresa}${p.marca ? ` · ${p.marca}` : ''}\n${p.complemento ? `Embalagem: ${p.complemento}\n` : ''}${p.peso ? `Peso: ${p.peso}\n` : ''}${p.desc || ''}`;

    if (navigator.share) {
      try {
        const payload = { title: p.nome, text, url: link };
        const file = await fileFromImage(imageUrl, String(p.cod).replace(/[^a-z0-9_-]/gi, '-'));
  ...
  async function shareCatalog() {
    ...
    const text = 'Catálogo Comercial — Acreaves, Dom Porquito e Fripal';
    if (navigator.share) {
  ...
  function setupModalSwipe() {
    ...
    modal.addEventListener('touchstart', (e) => {
      if (!state.current) return;
      if (e.target.closest('.modal-thumbs, .modal-actions, .modal-close, button, a')) return;
    ...
    modal.addEventListener('touchmove', (e) => {
      if (!tracking || !state.current) return;
      const t = e.changedTouches[0];
      ...
      if (Math.abs(dx) > 56 && Math.abs(dx) > Math.abs(dy) * 1.15) {
        tracking = false;
        openSiblingProduct(dx < 0 ? 1 : -1);
      }
    }, { passive: true });

    modal.addEventListener('touchend', () => { tracking = false; }, { passive: true });
  ...
  function setup() {
    ...
    const sentinel = new IntersectionObserver(entries => {
      if (entries[0]?.isIntersecting && !$('#load-more').hidden && state.screen === 'catalogo') {
        state.visible += PAGE_SIZE;
        renderProducts();
      }
    }, { rootMargin: '300px' });
    sentinel.observe($('#load-sentinel'));
    ...
    document.addEventListener('keydown', e => {
      ...
      if (!state.current) return;
      if (e.key === 'ArrowRight' && state.galleryIndex < state.current.imgs.length - 1) setGallery(state.galleryIndex + 1);
      if (e.key === 'ArrowLeft' && state.galleryIndex > 0) setGallery(state.galleryIndex - 1);
    });

    $('#modal-whatsapp').addEventListener('click', shareProduct);
    $('#modal-copy').addEventListener('click', copyProduct);
    ...
    if (code) {
      setTimeout(() => openProduct(code), 80);
    } else {
